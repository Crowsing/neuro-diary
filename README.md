...
| `SYNC=off` | звузити до local-only: жодного мережевого коду в бандлі |
| `REMINDERS=off` | прибрати екран нагадувань — форма, якою пішов би продакшен |
| `NGROK_DOMAIN`, `NGROK_BIN` | зарезервований домен і шлях до бінарника ngrok |

**За замовчуванням стенд повний**, тож наведеної вище команди досить, щоб пройти
шлях Mini App → api → БД із телефона. api власного публічного домену не потребує:
dev-сервер проксує `/v1/...` і `/health` на `localhost:$API_PORT`, тож web і api
— **одне походження**. Це знімає і вимогу другого тунелю (акаунт із одним доменом
його не дасть: ngrok віддав би обом ту саму адресу, і Mini App отримував би
`index.html` замість відповіді api), і CORS разом із ним.

Ціна цієї зручності названа прямо: за проксі стенд **не перевіряє CORS і
preflight**. Саме там жила відсутність `PUT` у `allow_methods` — дефект, якого не
видно нізвідки, крім крос-origin прогону. Тому job `sync-e2e` у CI лишається
крос-origin (`localhost:4174` проти `localhost:8000`) і проксі не використовує.

`REMINDERS=off` показує ту форму збірки, якою пішов би продакшен: екрана
нагадувань у ній немає **фізично**. Gate
[future-telegram-reminders.md](docs/plans/future-telegram-reminders.md) лишається
відкритим у двох людських пунктах (тексти згод `0.9` без імені контролера і
непідписаний review реальної доставки), тож §10 вимагає, щоб чесний стан
«Нагадування недоступні» лишався в UI. Обидві гілки перевіряються на артефакті —
`apps/web/scripts/assert-bundle.mjs` червоніє і на відсутності шляху там, де він
має бути, і на присутності там, де не має.
...
Mini App відкривається одразу у **фулскріні** — застосунок просить його сам, без
жодного перемикача. Щоб побачити це на пристрої: `./scripts/dev-start.sh
--set-menu-button`, далі кнопка «Щоденник» у боті. Геометрію фулскріна можна
відтворити й у звичайному браузері, без Telegram, — як саме, і чому інсети
поєднуються через `max()`, а не додаванням, описано в
[docs/twa-shell.md](docs/twa-shell.md).
