...
- [x] API pytest і Ruff зелені. Evidence: `uv run --locked pytest` — **873
...
**Друга лінза дала одну серйозну знахідку й чотири дрібні** — і найважливіша з
них про те саме, що й перша лінза: твердження, якого код не виконує.

1. **Серйозна: бар'єрна половина §11 на `POST /v1/sync/push` не тримається жодним
   тестом.** Зняття кроку 4 порядку §9.1 давало **нуль** нових червоних на всьому
   наборі. Це рівно те твердження, яке докстрінг
   `test_medical_consent_gate.py` робив про всі медичні ендпоінти: «кожна половина
   має тест, що падає лише від її зняття». Для push воно було хибним: двері
   відмовляють 403 незалежно від того, що робить сервіс, тож жоден HTTP-тест двох
   конструкцій не розрізняв — а це саме той ендпоінт, який записує щоденник, і
   саме та половина, що стоїть під `accounts.lock`, тим самим, який бере
   транзакція стирання §9.8. Додано два тести (згода і статус акаунта); кожен
   червоніє лише від зняття своєї половини.
2. **Дрібна:** `POST /v1/sessions/revoke-others` і `POST /v1/account/vault-reset`
   мали лише структурний тест — «bucket і рядок у таблиці», а не «bucket і 429».
   Додано поведінкові.
3. **Дрібна:** per-IP вікно §11 не мало поведінкового тесту у **справжньому**
   застосунку: зняття реєстрації `RateLimitMiddleware` червонило лише структурний
   тест, а механіку перевіряв юніт на власноруч зібраному стеку. Додано.
4. **Дрібна:** `schemathesis.toml` писав «глобально 422 не додано свідомо», ніби
   перелічення операцій зберігає клас знахідок «схема слабша за код». На цій
   поверхні не зберігає: сім операцій із дозволом — це рівно всі, де 422 взагалі
   досяжний. Твердження виправлене, і названо, що справді тримає той клас
   (`test_schema_declarations.py` пінує патерни й межі в документі).
5. **Дрібна:** перевірка «акаунт ще активний» у push теж не трималася нічим —
   успадкована з `main`, але фаза мала б закрити її разом із рештою. Закрито тим
   самим тестом.
...
- [x] Тест кожного ліміту червоніє від зняття **самого ліміту**, перевірено
  зняттям коду — **по кожному bucket-у окремо**, а не лише колективно:

  | Знято | Червоних |
  |---|---|
  | `budget = None` для всіх (колективно) | **17** |
  | `sync` | 10 |
  | `account_ops` | 11 |
  | `reminders_settings` | 6 |
  | `key_read` | 3 |
  | `push_bytes` (витрата в роутері) | 1 |

  Модулі: `test_endpoint_limits.py`, `test_medical_consent_gate.py`,
  `test_sync_rate_limits.py`, `test_reminder_settings.py`,
  `test_rate_limit_surface.py`.

  **Дефект у власній перевірці, названий прямо:** перша спроба per-bucket
  перевірки для `sync` і `key_read` дала нуль червоних — бо мій скрипт шукав
  `budget=RateBucket.SYNC,` із комою, а в коді там дужка, тож він **нічого не
  змінював**. Перевірка, що не може впасти, — це той самий клас дефекту, який ця
  фаза шукає в тестах; повторено з асертом «файл справді змінився».
...
- [x] §11 «двічі перевірена згода» виконана на всіх п'яти медичних ендпоінтах.
  Evidence: `tests/integration/test_medical_consent_gate.py` — 18 тестів, кожна
  половина з тестом, що червоніє **лише** від її зняття. Дві з них додав
  незалежний review: бар'єрну половину `POST /v1/sync/push` не тримало ніщо. Знайдено на коді:
...
- **15 із 15 операцій пройшли**, 642 тест-кейси, нуль провалів (прогін після правок за незалежним review; до них — 632 кейси, теж нуль провалів).
...
- **Хибні спрацювання названі поіменно** в `schemathesis.toml`, кожне з
  ...
  кладе базу зон у домен); міжполеві правила `model_validator`, яких JSON Schema
  не виражає; `1.0` як `integer` за JSON Schema проти `strict=True`; 413 на
  рядку, який схемно валідний і не є base64.
- **Сам цей конфіг перевірений на глушіння**, а не лише пояснений: прогін із
  **прибраними** дозволами 422 дав рівно чотири випадки, і всі чотири — ті, що в
  конфігу названі. Але два з них були **не тією причиною**, яку конфіг спершу
  наводив: це міжполеві правила (`надгробок не несе payload`, `settings` лише для
  `telegram_reminders`), а не цілі числа й не домен. Пояснення, яке називає не ту
  причину, — те саме глушіння, лише ввічливе; причину додано.
...
- **UI нагадувань і UI відкликання згод у web** — свідомо поза Фазами 4–5.
  **Закрито Фазою 6, і по-різному:** UI відкликання зроблений і ввімкнений; UI
  нагадувань зроблений і **явно вимкнений прапорцем**, бо gate
  `future-telegram-reminders.md` лишається відкритим у двох людських пунктах.
  Див. розділ Фази 6.
- **Симптом «перезавантаження вкладки робить синхронізацію недосяжною»** лишається
  ...
  Telegram SDK — поза обсягом Фази 5. **Уточнено у Фазі 6: цей шлях не існує**,
  і причина не в обсязі — див. розділ Фази 6 і розділ 3.1 threat model.
...
## Фаза 6 — UI нагадувань за прапорцем і UI відкликання згод (виконана)

Scope: web-бік нагадувань і відкликання згод, звірка gate
[future-telegram-reminders.md](future-telegram-reminders.md) з фактичним станом
коду, і закриття симптому «перезавантаження вкладки вбиває синхронізацію».

**Головне, що змінилося: gate звірено, і звірка змінила висновок.** Промпт фази
виходив із того, що quiet hours не мають погодженого діапазону. Код каже інше —
діапазон ухвалений Gate D, живе константами домену, експортується спільною
фікстурою й названий дослівно в самому тексті згоди `0.9`. Із семи пунктів gate
код закрив чотири повністю й один частково; відкритими лишаються два, і обидва
людські. Тому UI нагадувань **реалізований і явно вимкнений прапорцем**, а не
ввімкнений: пункт 7 gate («чесний unavailable-state лишається в UI, доки весь
delivery path не пройде ці gate-и») чинний. Повна таблиця — розділ «Звірка з
кодом (Фаза 6)» у самому документі gate.

### DoD

- [x] Звірка gate із кодом, із поіменними тестами на кожен закритий пункт і
  названою розбіжністю. Evidence: розділ «Звірка з кодом (Фаза 6)» у
  [future-telegram-reminders.md](future-telegram-reminders.md).
- [x] Шлях нагадувань реалізований і вимкнений прапорцем `VITE_REMINDERS`
  (default `off`), причому **обидві** гілки під тестом на артефакті. Evidence:
  `apps/web/scripts/assert-bundle.mjs` — без `--reminders` відмовляє на
  `/v1/reminders/`, з ним вимагає `/v1/reminders/settings`. Перевірено зняттям:
  `assert-bundle.mjs dist-sync --sync --reminders` на збірці без прапорця
  червоніє («the flag did not reach the bundle»).
- [x] Прапорець усуває **шлях, а не лише кнопку**. Evidence: у `dist-sync` без
  `VITE_REMINDERS` немає ані рядка `/v1/reminders/`, ані чанка
  `RemindersDialog` — модуль лишається без жодного споживача. Це коштувало
  окремої правки: перша редакція тримала методи в `SyncTransport`, тобто рядок
  адреси потрапляв у бандл безумовно, і гейт це впіймав.
- [x] Три форми збірки покриваються без жодного зайвого прогону в CI: `dist`
  (local-only), `dist-sync` без нагадувань у job `web` — та сама форма, якою
  пішов би продакшен, — і `dist-sync` із нагадуваннями у job `sync-e2e`.
- [x] Local-only гейт посилено із переліку трьох шляхів до суцільної відмови на
  `/v1/`. Причина: перелік, що росте разом з API, відстає від нього — і саме
  тому пропустив би `/v1/consents/revoke` та `/v1/reminders/settings`.
- [x] UI відкликання згод — без прапорця, Art. 7(3). Показує чинні згоди,
  дослівний текст відкликання з реєстру, і розрізняє три відповіді сервера:
  409 `confirm_required`, `account_erased: true`, 409 `consent_precondition`.
  Evidence: `apps/web/e2e/sync-consents.spec.ts`.
- [x] Відкликання **не** чіпає локальних даних (§9.4). Evidence:
  `sync-consents.spec.ts::відкликання health_sync не забирає локальних записів`
  — після erasure акаунта читає `nd_demo_v3` і асертить сам запис із нотаткою.
- [x] Тексти відкликання нарешті доходять до застосунку: плагін збірки мапив
  лише `registry.grants`, тепер і `revocations`. Evidence:
  `apps/web/src/sync/consentCopy.test.ts::lists a revocation text for each of the
  three consents`.
- [x] `fixtures/contract/quiet-hours.json` має web-споживача, і виняток у
  `fixtures/contract/README.md` знято. Evidence:
  `apps/web/src/sync/fixtures.test.ts::судить кожну межу quiet hours так само, як
  api` — асертить сам перелік меж, не їхню кількість.
- [x] Симптом «перезавантаження вкладки вбиває синхронізацію» — розібраний, а не
  закритий. Evidence: `apps/web/src/sync/initdata.test.ts::після F5 у SDK не
  лишається чого віддати — обидва його джерела зачищені`; розділ 3.1
  [threat-model.md](../threat-model.md) переписаний із трьома причинами.
- [x] Ціна симптому для користувачки зменшена: фраза більше не питається перед
  наперед відомою відмовою. Evidence: `SyncProvider` тримає `initDataPresent`,
  і `syncNow` за `false` веде одразу в `errorStepUp`.
- [x] `reminder-contract.spec.ts` і `production-controls.spec.ts` **не
  правилися** й лишилися зеленими. Це не збіг: гілка «вимкнено» рендерить ту
  саму картку дослівно, а обидва тести живуть у local-only проєкті.

### Зафіксовані рішення

- **Стенд за замовчуванням повний, а api їде тим самим тунелем.** Раніше
  `dev-start.sh` починав із local-only, а за `SYNC=on` шукав api окремий
  публічний домен — якого акаунт ngrok з одним доменом не дає. Скрипт це
  розпізнавав і вироджувався в `http://localhost:8000`, тобто пройти шлях
  Mini App → api → БД **з телефона було неможливо взагалі**. Тепер dev-сервер
  проксує `/v1/...` і `/health` на api, web і api стають одним походженням, і
  `./scripts/dev-start.sh --set-menu-button` достатньо. Ціна названа в
  `vite.config.ts` і README: за проксі стенд не перевіряє CORS і preflight —
  саме той шлях, на якому знайшлася відсутність `PUT`, — тож job `sync-e2e`
  лишається крос-origin і проксі не використовує.
- **Банер стенду більше не мовчить за порожнього `VITE_API_ORIGIN`.** Умова була
  `VITE_SYNC !== 'on' || API_ORIGIN === ''`, і друга половина глушила банер саме
  там, де він найпотрібніший: на стенді за проксі. Тепер умова — рівно прапорець
  sync, а порожнє походження означає `/health` того самого хоста. У local-only
  збірці прапорець вимкнений, тож запиту й далі немає. Evidence:
  `e2e/sync-two-devices.spec.ts::банер стенду видимий, бо APP_ENV=development`.
- **Одна сесія на вкладку — межа протоколу, а не зручність.** initData купує
  рівно одну сесію (anti-replay §8), тож окремий транспорт для нагадувань
  отримав би 401 на тому самому рядку. Звідси форма: `VaultSession` володіє
  сесією, `src/reminders/transport.ts` — адресою ресурсу, а порт складається в
  провайдері. Evidence: `vault.test.ts::другий вхід у тій самій вкладці не
  витрачає initData вдруге` (асертить `usedInitData.size === 1`); зняття гарда
  червонить два тести.
- **`SyncError` навчився нести серверний код.** `code` зводить 404 до
  `no_vault_key`, а обидва 422 — до `server`; нагадуванням треба розрізняти
  `no_schedule`, `quiet_hours_violation` і `unknown_timezone`, бо це три різні
  дії користувачки. Evidence: `src/reminders/client.test.ts`, розділ
  «розрізнення серверних відмов».
- **`503 consent_copy_not_frozen` має власний текст.** Сказати «не вдалося» там,
  де сервер сказав «текст не заморожений», означало б сховати єдину справжню
  причину, з якої функція недоступна поза стендом.
- **Порт рушія звужено до `push`/`pull`.** Розширення сусіднього ресурсу псувало
  заглушки в `engine.test.ts`, які про нього нічого не знають; `EngineTransport`
  робить залежність чесною.
- **Клієнт не витрачає спроб §11 на відомі відповіді.** Почувши `no_account`,
  сесія більше не питає те саме вдруге перед grant. Знайдено прогоном sync-e2e:
  вся сюїта разом перевищувала десять спроб на хвилину, і CI був би флакі.
  Evidence: `vault.test.ts::після почутого no_account другий вхід іде одразу з
  grant` плюс зворотний `::без попереднього no_account порядок §8 лишається
  двокроковим`.

### Відхилення від плану

- **Вісімнадцяте: `PUT` бракувало в CORS.** `allow_methods` містив лише
  `GET, POST`, тоді як §10 рутить `PUT /v1/reminders/settings`. Preflight
  відповідав без `PUT`, тож збереження розкладу з браузера було неможливе — на
  стенді й у CI web і api завжди на різних origin. Дефект існував із Фази 4 і не
  проявлявся, бо клієнт ходив лише `GET`/`POST`. Перелік більше не пишеться
  руками: `test_openapi_surface.py::test_every_routed_method_survives_a_browser_preflight`
  виводить його з фактично зарутованих операцій.

### Наскрізні гейти

Локальний прогін від 2026-07-27:

- `pnpm -C apps/web test` — 39 файлів, 601 тест. Зелено (базова лінія — 36 файлів,
  551 тест).
- `pnpm -C apps/web build` + `assert-bundle.mjs dist` — зелено.
- `pnpm -C apps/web e2e` — 82 passed, 2 skipped. Зелено, без правок наявних
  специфікацій.
- `assert-bundle.mjs dist-sync --sync --api=…` і те саме з `--reminders` на
  відповідних збірках — зелено; перехресна перевірка червоніє, як і має.
- `SYNC_E2E=1 playwright test --project=sync` проти локального стенду з реальною
  PostgreSQL — **10 passed** (було 7; Фаза 6 додала три). Прогін обов'язковий:
  змінилися і клієнт, і CORS api. Він же й знайшов три дефекти, яких локальний
  прогін не бачить: пропущену автентифікацію перед першим читанням розкладу,
  відсутній `PUT` у CORS і перевищення §11 усією сюїтою разом.
- `cd apps/api && pytest` — базова лінія до правок **873 passed, 2 skipped,
  3 failed**; після — **876 passed, 2 skipped, 3 failed**. Приріст — три тести
  preflight; склад червоних не змінився: усі три — `test_telegram_surface.py`
  про ім'я бота (§10), і вони червоні з тієї самої причини, що й були.
- `ruff check`, `ruff format --check`, `mypy --strict app`, `lint-imports` —
  зелено, 7 контрактів цілі.

### Що лишилося за людиною

Без змін до Фази 5, крім двох рядків:

- **UI нагадувань лишається вимкненим.** Умова вмикання рівно одна й не кодова:
  заморозити тексти згод до `1.0`, назвавши контролера, і зібрати підписи
  privacy/security/clinical review реальної доставки. Після цього
  `VITE_REMINDERS=on` — і більше нічого.
- **Симптом «перезавантаження вкладки»** лишається відкритим, але вже не як
  відкладена робота: названий шлях не існує (розділ 3.1 threat model).
