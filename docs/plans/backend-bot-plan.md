...
- Покриття рахується як `min(now − service_start_at, J)`, а **не** як `now − min(at)` —
  ...
  **Джерело `service_start_at` (уточнено у Фазі 3).** Попередня редакція називала величину
  у формулі, не називаючи, звідки вона береться, — а вона не існувала ані в конфігу, ані в
  БД, ані в коді, тож інтерлок був нереалізовним. Це **константа деплою**
  (`SERVICE_START_AT`), а не рядок у БД: інтерлок оцінюється під час інциденту
  відновлення, і його входами мають бути конфіг (бік деплою) та бакет журналу (інший
  провайдер) — жоден із них не є тією базою, яку інтерлок охороняє. Рядок у БД зробив би
  guard залежним від артефакта, який він перевіряє; «найраніший об'єкт у бакеті»
  вироджується рівно в `now − min(at)`, яку цей же пункт відкидає. Ціна названа: значення
  ставиться один раз і ніколи не змінюється, а помилка оператора дає fail-closed
  блокування restore. Майбутня дата дає нульове покриття.
...
**Писачів `security_reset` два, а не один (уточнено у Фазі 3).** Попередня редакція
припускала один момент запису, і будь-який один момент лишає вікно. §7 задає порядок
`vault-reset → upload → конверт`, тобто операція розтягнута в часі. Бекап, знятий **до**
`vault-reset`, покривається записом від `vault-reset`. Але бекап, знятий **між**
`vault-reset` і записом конверта, містить **старий конверт** — `vault-reset` свідомо не
чіпає `vault_key`, бо він і є першою половиною re-key — тоді як запис від `vault-reset`
для нього вже застарий (`at < t_b`) і не реконсилюється. Це вікно закриває лише запис від
самого `POST /v1/sync/key {mode:"rekey"}`. Сервер до того ж не примушує порядок §7:
re-key можна викликати без попереднього `vault-reset`. Тому журналюють обидві операції.
`vault-reset` — безумовно: сервер не відрізняє скидання порожнього сейфа від першої
половини re-key, і зайвий запис коштує один ідемпотентний no-op, тоді як пропущений
лишає старий конверт живим після відновлення. Запис у `/v1/sync/key` робиться **після**
перевірки CAS і **до** запису конверта: 409 не має лишати сліду про дію, якої не було.
...
- **Матриця erasure іменована по кожній із 14 таблиць**, а не задана числом. Окремої уваги
  ...
  **Число виправлено у Фазі 3 з 12 на 14.** Попередня редакція вимагала переліку імен і в
  тому самому реченні задавала матрицю числом — а число не вміє помітити нову таблицю.
  Міграції створюють чотирнадцять: `rate_window` додала Фаза 2 (зафіксовано окремим
  відхиленням), `message_cleanup` — міграція `0002`. Разом із числом додано дві особливі
  таблиці, яких попередня редакція не називала. **`erasure_job` — це і є журнал**: він
  свідомо переживає акаунт (FK на `account` у нього немає саме тому), і «нуль рядків» для
  нього хибне за побудовою. **`reminders.message_cleanup`** не має FK, не має ORM-моделі й
  не видаляється жодним шляхом erasure — `api_rw` має на неї лише INSERT; її рядок
  закривається жорстким TTL 48 год, писач і споживач якого належать Фазі 4. Сам перелік
  живе в [restore-runbook.md](../restore-runbook.md) і звіряється з `information_schema`
  тестом, тож наступна міграція червонить CI, доки матриця не назве нову таблицю.
...
- **Другий виняток із manifest, симетричний першому (уточнено у Фазі 3): записи, які сервер має право вилучити за названим ключем (§9.7), не входять до manifest ані при побудові, ані при перевірці.** Сьогодні це рівно синглтон `cycle`. Причина та сама, що й для надгробків, і виявлена тим самим способом: §9.7 вимагає від сервера hard-DELETE запису `cycle` без надгробка, після чого manifest, зафіксований до відкликання, покриває запис, якого на сервері вже немає. Кожен наступний повний ресинк давав би нейтральну помилку «серверна копія виглядає застарілою» без жодної атаки, і сам себе цей стан не лікує: перевірка виконується **до** застосування сторінки, тож pull ніколи не завершується, а push, який міг би перевидати manifest, у штатному циклі `pull → merge → push` до виконання не доходить. Вихід у користувачки лишався б один — re-key, тобто повна втрата серверної копії через штатне відкликання згоди.
- **Перелік виключених шляхів — статична константа клієнта, а не сигнал із відповіді сервера.** Це і є причина, з якої відкинуто очевидніший варіант «сервер піднімає `consent_epoch`, клієнт перевидає manifest до першого повного ресинку»: `consent_state_changed` контролює сервер, отже він сам вирішував би, коли клієнт пропустить перевірку цілісності — і anti-rollback зникав би для **всього** сейфа за бажанням сервера, а не лише для одного синглтона. Варіант «клієнт виключає запис із уже підписаного MAC» неможливий механічно: manifest — HMAC над канонічним рядком усього набору, а не Merkle-дерево, тож «вилучити елемент» на боці перевіряльника не можна, і саме тому compacted-надгробки закриті виключенням при **побудові**. Статична константа заразом знімає питання «як клієнт дізнається, що саме вилучено, не дізнавшись назви згоди з pull (§9.2)»: дізнаватися нема чого.
- **Ціна цього винятку названа прямо: запис `cycle` втрачає anti-rollback-покриття manifest.** Сервер може приховати його або віддати старішу версію, і manifest цього не викриє. Чотири механізми, що лишаються чинними, роблять цю втрату вузькою: перевірка `HMAC(k_index, шлях) == record_key` не дає підсунути під цим ключем чужий запис; AAD прив'язує ciphertext до `client_ts_ms`, тож мітку не підмінити; LWW за автентифікованим `client_ts` зсередини payload (§9.3) не дає старішій версії перезаписати свіжішу локальну; а виняток §9.4 для неактивних згод уже забороняє видаляти локальні дані домену, чия згода неактивна — тобто приховування `cycle` сервером і до цієї правки не давало ефекту, заради якого його робили б. Залишковий ризик — новий пристрій, у якого локальної копії циклу немає взагалі: він отримає старішу версію без сигналу. Записано в §13.
...
- **Tiebreaker поелементного merge** (уточнено у Фазі 2): при РІВНИХ мітках і різному вмісті перемагає лексикографічно більший канонічний рядок елемента (ключі об'єктів упорядковані). Правило `tie → remove` покриває лише add-vs-remove; для add-vs-add із рівними мітками попередня редакція правила не давала, і реалізація виявилася залежною від порядку аргументів — тобто два пристрої з однаковим годинником розходилися назавжди. Те саме правило застосовується до пари `order`/`order_at` синглтонів `catalog` і `groups`, і з тієї самої причини. Альтернатива «покласти `device_id` у кожен поелементний запис» відхилена: вона змінює формат payload, а виграшу над порівнянням вмісту не дає. Окремо зафіксовано: список `order` **не** фільтрується за наявними елементами під час merge (фільтрує лише десеріалізація в домен) — фільтр робив merge неасоціативним, і id зникав із порядку назавжди після тристороннього злиття.
...
- Клієнт надсилає при grant `cycle_sync` значення `record_key_cycle = HMAC(k_index,'cycle')`, яке зберігається в рядку згоди. **Колонка `consent.record_key_cycle` додається міграцією Фази 2, а не `0002`:** grant-контракт §9.2 (свіжіше рішення Gate D) її не містить, у DDL §6.2 її немає, а клієнт уперше вміє обчислити `HMAC(k_index,'cycle')` лише у Фазі 2, коли з'являється `k_index`. Єдиний споживач — серверний hard-DELETE за названим ключем — лишається Фазі 3: колонка заводиться разом із прийманням значення, поведінка видалення — окремо. Тегувати кожен запис колонкою `domain` не потрібно: витік той самий (один біт), але вартість нульова — нуль змін AAD, нуль змін DTO push, нуль `CHECK`, який довелося б мігрувати при появі другого домену.
...
- **Наслідок для manifest (уточнено у Фазі 3).** Оскільки серверне видалення за названим ключем робить серверний набір строгою підмножиною зафіксованого — рівно так само, як це робив би компактор із надгробками, — записи named-key доменів виключено з manifest симетрично на обох боках (§7). Без цієї правки штатне відкликання `cycle_sync` перетворювало б кожен наступний повний ресинк на постійну помилку цілісності.
- **Якщо `record_key_cycle` відсутній** (згоду дано до появи колонки, або клієнт не надіслав значення), видаляти нема чого і сервер нічого не видаляє: відкликання проходить, `revoked_at` виставляється, ревізія **не** інкрементується. Мовчазним це бути не може — саме цей випадок робить обіцянку §9.7 невиконаною для конкретного акаунта, тож він рахується окремою метрикою і лишається чесно описаним: повний серверний enforcement за E2E неможливий доти, доки клієнт не назвав ключ. Вигадувати «видалити щось схоже» сервер не має права — він не бачить, який ciphertext є циклом.
...
5. `cycle_sync` — **переформульовано після Gate D:** фільтрація на push лишається клієнтською, але відкликання виконує сервер hard-DELETE за `record_key_cycle`, який клієнт назвав при grant (§9.7). Залишковий ризик звужується до одного біта: сервер знає, який `record_key` є циклом, у акаунтів, що надали цю згоду. Опція на майбутнє — окремий крипто-домен/DEK. **Доповнено у Фазі 3:** ціною узгодження §7 із цим видаленням запис `cycle` виведено з-під anti-rollback-покриття manifest (§7). Приховування або відкат цього запису сервером не викривається manifest'ом; для пристрою з локальною копією наслідків немає (LWW за автентифікованим `client_ts` і виняток §9.4 для неактивних згод), для нового пристрою без локальної копії — він отримає старішу версію циклу без сигналу. Прийнято свідомо: альтернативи або віддавали серверу право вимикати перевірку цілісності всього сейфа, або лишали штатне відкликання згоди станом, з якого виходять лише через re-key.
